(() => {
  ...
  function chooseCurrentWeek(rows) {
    ...
    // У SiteData тиждень НЕ зберігається. Сайт визначає його календарно: Пн–Нд.
    // Якщо week_start/week_end випадково присутні у CSV, їх можна використати,
    // але вони не є обов'язковими.
    const hasWeek = normalized.some(r => r.week_start);
    if (!hasWeek) return normalized;

    const wanted = currentWeek();
    const exact = normalized.filter(r => r.week_start === wanted.start && r.week_end === wanted.end);
    if (exact.length) return exact;

    const dated = normalized.filter(r => r.week_start);
    if (!dated.length) return normalized;
    const starts = [...new Set(dated.map(r => r.week_start))].sort().reverse();
    return dated.filter(r => r.week_start === starts[0]);
  }
  ...
  function render(rows) {
    ...
    if (items.length) {
      // Діапазон завжди береться з календаря поточного тижня (Пн–Нд).
      // Дані з SiteData не повинні містити дати.
      const week = currentWeek();
      dateEl.textContent = formatRange(week.start, week.end);
    }
  }
